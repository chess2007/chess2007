\ Переместить символ в строке ( массиве байт ) a u  из позиции n1 в позицию n2
\                1 2 3  4
: carry-symbol ( a u n1 n2 -- a u )
4/13+b34<?13+i13+e14+dit34-aV14+w12 ;

\ вариант с операторами в стандартном виде
: car-symb ( a u n1 n2 -- a u )
4/13 + C@ /34 < IF /13 + 1+ /13 + ELSE /14 + DUP 1+ THEN /34 - ABS MOVE /14 + C! /12 ;

\ можно также использовать смешанный вариант - произвольное число сокращенных операторов и стандартных операторов в любой их комбинации
: car-symb1 ( a u n1 n2 -- a u )
4/13+ C@ /34< IF /13+ 1+ /13+ /e /14+ DUP 1+ /t /34- ABS MOVE /14+ C! /12 ;

\ Поменять местами символы в строке ( массиве байт ) a u  из позиции n1 и из позиции n2
